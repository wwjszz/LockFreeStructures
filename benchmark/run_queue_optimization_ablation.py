...
VARIANTS = (
    ...
    ("direct_dispatch", "Optimization 2 · direct token dispatch", 2),
    ("word_flags", "Optimization 3 · direct dispatch + word flags", 3),
    ("all_optimizations", "Consumer cache + direct dispatch + word flags", 4),
    ("moodycamel", "moodycamel", 5),
)
...
SCENARIO_SETS = {
    "balanced": BALANCED_SCENARIOS,
    "fan_in": FAN_IN_SCENARIOS,
}
...
def compiler_name(compiler: str, root: Path) -> str:
    if Path(compiler).name.lower() in {"cl", "cl.exe"}:
        ...
        return f"MSVC {version.group(1)}" if version else "MSVC"
...
def make_original_tree(root: Path, destination: Path, baseline_ref: str) -> None:
    archive = destination.parent / "original.tar"
    run(
        ["git", "archive", "--format=tar", f"--output={archive}", baseline_ref, "ConcurrentQueue", "common"],
        cwd=root,
    )
...
def build_variants(
    ...
    for name, _, variant in variants:
        ...
        binary = binary_directory / (f"{name}.exe" if is_msvc else name)
        if is_msvc:
            command = [
                compiler,
                "/nologo",
                ...
                str(source),
                f"/Fe:{binary}",
                f"/Fo:{binary_directory / (name + '.obj')}",
            ]
        else:
            command = [
                compiler,
...
def collect(
    ...
    variants: tuple[tuple[str, str, int], ...],
    paths: tuple[str, ...],
    scenarios: tuple[tuple[int, int, int], ...],
    repetitions: int,
...
def main() -> None:
    ...
    parser.add_argument(
        "--baseline-ref",
        default="custom_dev",
        help="git ref containing the original queue headers",
    )
    ...
    parser.add_argument(
        ...
        default="balanced",
        help="thread topology to benchmark",
    )
    parser.add_argument(
        "--output",
        type=Path,
    ...
    scenarios = SCENARIO_SETS[arguments.scenario_set]
    baseline_commit = run(
        ["git", "rev-parse", arguments.baseline_ref], cwd=root, capture=True
    )
    ...
    payload = {
        ...
        "definitions": {
            "original": "queue headers from the recorded baseline git ref",
            "consumer_cache": "thread-local token-less consumer cache only; legacy ProducerToken dispatch and byte flags",
            "direct_dispatch": "direct ProducerToken dispatch; consumer cache disabled; byte flags",
            "word_flags": "direct ProducerToken dispatch plus WordFlagsCheckPolicy; consumer cache disabled",
            "all_optimizations": "consumer cache and direct ProducerToken dispatch with opt-in WordFlagsCheckPolicy",
