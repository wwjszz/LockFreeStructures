...
//   2: direct ProducerToken dispatch only
//   3: direct ProducerToken dispatch + packed word flags
//   4: all optimizations
//   5: moodycamel
...
#if HAKLE_ABLATION_VARIANT == 2
struct DirectDispatchTraits : hakle::ConcurrentQueueDefaultTraits<int, BenchAllocator> {
    static constexpr bool UseImplicitConsumerCache = false;
};

using SelectedHakleQueue = hakle::ConcurrentQueue<int, BenchAllocator, DirectDispatchTraits>;
#elif HAKLE_ABLATION_VARIANT == 3 || HAKLE_ABLATION_VARIANT == 4
struct WordFlagsTraits : hakle::ConcurrentQueueDefaultTraits<int, BenchAllocator> {
    ...
    static constexpr bool UseImplicitConsumerCache = HAKLE_ABLATION_VARIANT == 4;
