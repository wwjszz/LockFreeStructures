...
ALL_SERIES = (
    ...
    ("direct_dispatch", "Optimization 2 · direct dispatch", "#E69F00", "triangle", ""),
    ("word_flags", "Optimization 3 · direct dispatch + word flags", "#CC79A7", "triangle", ""),
    ("optimized_default", "Optimized Hakle", "#009E73", "diamond", ""),
    ("all_optimizations", "Optimized Hakle + WordFlags", "#CC79A7", "diamond", ""),
...
def render(input_path: Path, output_path: Path, series_names: tuple[str, ...]) -> None:
    ...
    configurations_in_results = {
        (str(row["path"]), int(row["producers"]), int(row["consumers"]))
        for row in rows
    }
    for path, producers, consumers in configurations_in_results:
        source = "consumer_cache" if path.startswith("implicit") else "direct_dispatch"
        source_key = (source, path, producers, consumers)
        if source_key in values:
            values[("optimized_default", path, producers, consumers)] = values[source_key]
    requested = set(series_names)
    series = tuple(entry for entry in ALL_SERIES if entry[0] in requested)
    if len(series) != len(requested):
        raise SystemExit(f"unknown series: {sorted(requested - {entry[0] for entry in series})}")
    ...
    title = "ConcurrentQueue throughput · original vs optimized"
    subtitle = (
    ...
    parts = [
        ...
        '<desc id="chart-desc">Three throughput plots compare the original Hakle queue, the optimized default Hakle hot paths, and moodycamel using implicit, token, and token bulk workloads.</desc>',
        """<style>
...
def main() -> None:
    ...
    parser.add_argument(
        "--series",
        nargs="+",
        choices=[entry[0] for entry in ALL_SERIES],
        default=["original", "optimized_default", "moodycamel"],
        help="series to include; the default is the README comparison",
    )
    arguments = parser.parse_args()
    render(arguments.input, arguments.output, tuple(arguments.series))
